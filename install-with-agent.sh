...
install.sh as the canonical recipe; do the steps yourself with your file
and Bash tools so you can adapt to the user's answers and recover from
errors.

Begin by:
  1. Reading INSTALL.md end-to-end.
  2. Reading install.sh end-to-end.
  3. Reading install/README.md.
...
Important context for this session:
  ...
  - Other harnesses (the ones not in the list above) may need to be
    installed if the user picks them.  Step 7b in install.sh has the
