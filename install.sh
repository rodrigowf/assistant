...
EOF
            ...
            # Create .env template — only includes the keys the user's chosen
            # axes actually need.  Each commented block explains where to get
            # the key and which features it unlocks.
            {
                echo "# Personal Assistant Environment Configuration"
                echo "# Generated by install.sh — fill in the keys for the axes you selected."
                echo ""

                if [ "$WITH_OPENAI" = true ]; then
                    echo "# OpenAI — orchestrator text mode (GPT-*), Realtime voice"
                    echo "# (gpt-realtime), and the OpenAI-compatible endpoint we use for"
                    echo "# Gemini models too.  Required if you'll use any of those."
                ...
                if [ "$WITH_ANTHROPIC" = true ]; then
                    echo "# Anthropic — orchestrator's Claude models.  Not needed if you"
                    echo "# only use the Claude Code harness (which has its own OAuth)."
                    echo "# Get yours at: https://console.anthropic.com/settings/keys"
                    echo "ANTHROPIC_API_KEY="
                    echo ""
                fi

                if [ "$WITH_QWEN" = true ] || [ "$WITH_OPENAI" = true ]; then
                    # Qwen harness needs DashScope; OpenAI orchestrator users
                    # may also want Qwen models via the OpenAI-compatible endpoint.
                    echo "# Alibaba DashScope — Qwen voice (orchestrator/providers/qwen_voice.py),"
                    echo "# Qwen orchestrator models via the OpenAI-compatible endpoint, and the"
                    echo "# Qwen CLI itself.  Get yours at: https://dashscope.console.aliyun.com"
                    echo "DASHSCOPE_API_KEY="
                    echo ""
                fi

                echo "# Session harness override.  The Configuration UI is the primary way"
                echo "# to flip this; the env var only applies when assistant_config.json"
                echo "# hasn't been written yet.  Options: claude, qwen"
                echo "# ASSISTANT_PROVIDER=$DEFAULT_PROVIDER"
                echo ""

                if [ "$WITH_QWEN" = true ]; then
                    echo "# Path to the qwen CLI — only set if \`qwen\` isn't on \$PATH for the"
                    echo "# user the backend runs as (e.g. NVM installs it under a versioned"
                    echo "# node directory)."
                    echo "# QWEN_CLI_PATH=/home/you/.nvm/versions/node/vXX.YY.ZZ/bin/qwen"
                    echo ""
                fi

                echo "# Default orchestrator model (UI can change this per session)."
                echo "# ORCHESTRATOR_MODEL=gpt-4o"
                echo ""
                echo "# Default realtime voice model."
                echo "# REALTIME_MODEL=gpt-realtime"
                echo ""
                echo "# Add your custom environment variables below"
            } > context/.env
